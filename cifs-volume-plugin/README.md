...
- The contents of `/root` is exposed to the plugin as a *read-only* mount so it may access the `/root/.netrc` file.
- The passwords are expected to be stored in `/root/.netrc` and it will verify that it is not accessible by anyone else.
